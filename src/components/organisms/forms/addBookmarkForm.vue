    ...
    >
      <div class="bg-gray-200 shadow-md rounded flex flex-col">
        ...
        <div class=" px-8 pt-6 pb-8 mb-4 ">
          <BookmarkForm :formid="'bookmarkForm'" />
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  computed: {},
  mounted() {
    this.$root.$on("closeBookmarkModal", data => {
      this.toggleModal();
    });

    this.$root.$on("sendBookmarkData", data => {
      console.log("bmark", data);
    });
  ...
  methods: {
    ...
    toggleModal() {
      const modal = this.$refs["modal-add-bookmark"];
